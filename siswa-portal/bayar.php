...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($isMonthly && empty($bulanInput)) {
        ...
        $validMonths = getBulanIndonesia();
        foreach ($bulanInput as $key => $bln) {
            if (!in_array($bln, $validMonths)) {
                unset($bulanInput[$key]);
            }
        }
                    ...
                    if (move_uploaded_file($_FILES['bukti_bayar']['tmp_name'], $uploadPath)) {
                        ...
                        try {
                            ...
                            if (file_exists($uploadPath)) {
                                unlink($uploadPath); // Bersihkan file bukti yang diupload jika DB gagal
                            }
                        ...
                        if ($isDbSuccess) {
                            try {
                                ...
                                // Abaikan log error jika fcm gagal, tidak boleh menyebabkan transaksi gagal
                                error_log("FCM Error pada bayar.php: " . $e->getMessage());
                            }
                        }
                    } else {
